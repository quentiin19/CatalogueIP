...
int add_ip_address(sqlite3 *db);
int display_all(sqlite3 *db);
int delete_ip_address(sqlite3 *db);
...
void convert_ipv4_to_binary(const char* ipv4_address, char* binary_result) {
    ...
    // Convertir chaque octet en binaire et ajouter les points séparateurs
    for (int i = 0; i < 8; i++) {
...
void convert_IPV4(const char* ipv4_address, char* binary_result, char* mask, char* binary_mask, char* hex_result, char* network) {
    ...
    // Calcul du réseau
    for (int i = 0; i < 36; i++) {
        network[i] = (binary_result[i] == '1' && binary_mask[i] == '1') ? '1' : '0';
    }
    network[36] = '\0';
}
